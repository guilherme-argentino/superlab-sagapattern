...
import com.github.slsp.app.config.KafkaProperties;
...
class SuperLabSagaPatternKafkaResourceIT {
    ...
    void setup() {
        ...
        SuperLabSagaPatternKafkaResource kafkaResource = new SuperLabSagaPatternKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/super-lab-saga-pattern-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/super-lab-saga-pattern-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
